...
class PolicyGradient(object):
    ...
    def __init__(self, env, config, seed, logger=None):
        ...
        self.discrete = isinstance(env.action_space, gym.spaces.discrete.Discrete)
    ...
    def sample_path(self, env, num_episodes=None):
        ...
        while num_episodes or t < self.config.batch_size or (t >= self.config.batch_size and not last_ep_finished):
            state = env.reset()[0]
            ...
            for step in range(self.config.max_ep_len):
                ...
                if done or step == self.config.max_ep_len - 1:
                    ...
                    last_ep_finished = t >= (self.config.batch_size - 1)
                    break
    ...
    def train(self):
        ...
        for t in range(self.config.num_batches):
            ...
            paths, total_rewards = self.sample_path(self.env)  # (#episodes, ()), (#episodes,)
            all_total_rewards.extend(total_rewards)
            observations = np.concatenate([path["observation"] for path in paths])  # (batch_size, *obs_space)
            actions = np.concatenate([path["action"] for path in paths])  # (batch_size,)
            rewards = np.concatenate([path["reward"] for path in paths])  # (batch_size,)
            # compute Q-val estimates (discounted future returns) for each time step
            returns = self.get_returns(paths)  # G_t (batch_size, 1)

            # advantage will depend on the baseline implementation
            advantages = self.calculate_advantage(returns, observations)  # (batch_size,)
