...
class PPO(PolicyGradient):
    ...
    def update_policy(self, observations, actions, advantages, old_logprobs):
        ...
        old_logprobs = np2torch(old_logprobs, device=self.config.device)

        #######################################################
        #########   YOUR CODE HERE - 10-15 lines.   ###########
        action_distributions = self.policy.action_distribution(observations)
        log_probs = action_distributions.log_prob(actions)
        r_theta = log_probs / old_logprobs
        clip = r_theta.clamp(min=1-self.eps_clip, max=1+self.eps_clip)

        values1 = r_theta * advantages
        values2 = clip * advantages
        min_values = torch.minimum(values1, values2)
